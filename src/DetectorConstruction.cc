...
:G4VUserDetectorConstruction(),
 world_mat(0), tracker_mat(0), shie_mat(0), shie_mat2_Pb(0), vach_mat(0), hewi_mat(0),
 Alch_mat(0), CoWa_mat(0), D2O(0)
{
  fLogicalTarget = new G4LogicalVolume*[3];
  DefineMaterials();
}
...
void DetectorConstruction::DefineMaterials()
{
  ...
  tracker_mat = nist->FindOrBuildMaterial("G4_Galactic");

  G4Material* shie_mat1_poly = nist->FindOrBuildMaterial("G4_POLYETHYLENE");
  ...
  //material:Boron cantaining polythelene
  density = 0.09983*g/cm3;
  shie_mat = new G4Material("Boron cantaining polythelene", density, ncomponents=2);
  shie_mat->AddMaterial(shie_mat1_poly, fractionmass=92.0*perCent);
  shie_mat->AddMaterial(shie_mat1_B4C,  fractionmass= 8.0*perCent);

  shie_mat2_Pb = nist->FindOrBuildMaterial("G4_Pb");

  vach_mat = nist->FindOrBuildMaterial("G4_Galactic");
  hewi_mat = nist->FindOrBuildMaterial("G4_Be");
  Alch_mat = nist->FindOrBuildMaterial("G4_Al");
  CoWa_mat = nist->FindOrBuildMaterial("G4_WATER");
  ...
  D2O = new G4Material("D2O", 1.11*g/cm3, ncomponents=2, kStateLiquid, 293.15*kelvin, 1*atmosphere);
